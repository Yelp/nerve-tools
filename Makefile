...
.PHONY: release
release:
	dch -v $(RELEASE) --distribution xenial --changelog src/debian/changelog $$'$(VERSION) tagged with \'make release\'\rCommit: $(LAST_COMMIT_MSG)'
	git ci -am 'Bump version'
	git tag $(RELEASE) master
	git show
	@echo Now run `git push origin $(RELEASE)` to release this version
